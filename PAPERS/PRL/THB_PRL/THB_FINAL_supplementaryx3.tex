...
\label{eq:4eqns}
\begin{align}
\frac{d \rho_{++}}{dt} &= i\frac{d_{+}}{2\hbar}(f^*\eta_{+g}-f\eta_{+g}^*), \label{eq:rho++}\\
...
It is much simpler to work with normalized quantities. Therefore henceforth we employ the notation $\Theta \rho_{jj} = n_j$, $i\Theta \eta_{\pm g} = P_{\pm}$, $\Theta \rho_{+-} = P_{+-}$ and finally we normalized the field to the Rabi frequency, i.e. $F= df/2\hbar$, where we have assumed $d_+\approx d_- = d$, also $\Theta = N\Gamma d^2\omega_0/(2\epsilon_0n_0^2\hbar)$ and the reset of the notations are as usual. Using the shortly introduced substitutions, we can rewrite Eqs. (\ref{eq:rho++})-(\ref{eq:eta-g}) as
\begin{subequations}
\label{eq:4eqns-v2}
\begin{align}
\frac{d n_+}{dt} &= \bar{F}P_+ + F\bar{P}_+, \label{eq:n+}\\
\frac{d n_-}{dt} &= \bar{F}P_- + F\bar{P}_-, \label{eq:n-}\\
\frac{d P_{+-}}{dt} &= -2i\Omega_{lr}P_{+-}+F\bar{P}_{-}+\bar{F}P_+,\\
\frac{d P_+}{dt} &= -i\Omega_{lr}P_+-F(n_{+}-n_{g})-FP_{+-}, \label{eq:P+}\\
\frac{d P_-}{dt} &= +i\Omega_{lr}P_--F(n_{-}-n_{g})-FP_{-+}, \label{eq:P+}
\end{align}
\end{subequations}
where we have omitted the time evolution of $n_g$ for the obvious reasons. Also the spatio-temporal evolution of the Rabi-frequency $F$ obeys the following propagation equation (derivable in the slowly varying amplitude approximation)
\begin{align}
\frac{\p F}{\p t} + \frac{c}{n_0}\frac{\p F}{\p x} &= -P_+-P-.  \label{eq:rabi-freq-normalframe}
\end{align}
where we have neglected optical losses. 

We can cast Eqs. (\ref{eq:4eqns-v2}) into their final form, i.e. Eqs. (\ref{MAIN-eq:4eqns}) from the main body of the article, via assuming that when resonance detuning is small, i.e. $\epsilon \approx  0$ , then the anticrossing splitting yields the energy difference $\omega_+-\omega_0 \approx \Omega_{lr}$ and $\omega_--\omega_0\approx - \Omega_{lr}$ as well as $\omega_+-\omega_- \approx 2\Omega_{lr}$. Next we assume that the dipoles have the same algebraic value, i.e. $d_+ = d_- =d$, and that the optical field's Rabi frequency evolves as a real number, i.e. $\beta(x,t) = df(x,t)/2\hbar \in \mathbb{R}$. 

Under these conditions we can decouple the system of ODEs in Eq. (\ref{eq:4eqns}) by setting $P = P_+ - \bar{P}_-$, $\Sigma = P_++\bar{P}_-$, $W = n_{+}-n_{-}$ and deriving the time-evolution of these quantities. After some simple algebraic operations we obtain
\begin{align}
\frac{d W}{dt} &= F(P+\bar{P}), \\
\frac{d P}{dt} &= -i\Omega_{lr}P-FW, \label{eq:eta}
\end{align}
for $W$ and $P$ as well as 
\begin{align}
\frac{d n_{g}}{dt} &= - F(\Sigma+\bar{\Sigma}), \\
\frac{d P_{+-}}{dt} &= -2i\Omega_{lr}P_{+-}+F\Sigma,\\
\frac{d \Sigma}{dt} &= -i\Omega_{lr}\Sigma-F(N-3n_{g})-2FP_{+-},
\end{align}
where $N = n_+ + n_- + n_g$. The propagation equation thus becomes  
\begin{align}
\frac{\p F}{\p t} + \frac{c}{n_0}\frac{\p F}{\p x} &= -\frac{P+\Sigma}{2}+\frac{\bar{P}-\bar{\Sigma}}{2} = -\frac{i}{2}\Im{P}-\frac{1}{2}\Re{\Sigma}.  \label{eq:rabi-freq-normalframe}
\end{align}

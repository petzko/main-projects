...
\documentclass[preprint,secnumarabic,amssymb, nobibnotes, aps, prd]{revtex4-1}
...
\end{align}
where we have defined $\gamma_{sg} =(\delta - \epsilon + i\Gamma_{\parallel sg}) $, $\gamma_{eg} =(\delta + i\Gamma_{\parallel eg})$, $\gamma_{se} = \epsilon - i\Gamma_{\parallel se}$ and $\delta = \omega_c-\omega_0$ is the detuning of the probe field from resonance. In the strong tunneling coupling regime, the populations of $\ket{s}$ and $\ket{e}$ are almost equal therefore the linear susceptibility simplifies to
\begin{align}
...
The introduction of the coupling energy $\hbar \Omega_{se}$ into the Hamiltonian of the system contributes to a splitting of the optical spectra into a high and a low frequency lobes. This is due to the fact that the extended system's Hamiltonian is non-diagonal in the tight binding basis. The new eigenstates, which diagonalize the Hamiltonian in Eq. (\ref{eq:hamiltonian-matrixform}), are the so called dressed states and are obtained from $\Ket{s}$ and $\Ket{e}$ via the unitary transformation
 ...
 \begin{figure}[h!]
 	\begin{center}
 		\includegraphics[scale=0.7]{BIASDETUNING.eps}
...
\section{Temporal hole burning due to strong injector anticrossing - 01}
...
\begin{widetext}
	...
	\frac{d\rho_{ee}^{+}}{dt} &= 2i\delta\omega\rho_{ee}^{+}  + i\Omega_{se}(\rho_{es}^{+}-\rho_{es}^{+}) + i\frac{q_0d_{eg}}{2\hbar} \left [ (f^{(-\delta)})^*\eta_{eg}^{(\delta)} -c.c.\right]
	+\Gamma_{se}\rho_{ss}^{+} + \Gamma_{ge}\rho_{gg}^{+} - \Gamma_e \rho_{ee}^{+}, \\
...
\section{Temporal hole burning due to strong injector anticrossing -02 - no time dependence}

As we said, we separate the (complex) envelope as well as the coherences into a positive and negative frequency components via the following ansatz 
\begin{subequations}
	\begin{align}
	\label{eq:splittingAnsatz}
	f &= f^{(\delta)}e^{-i\delta\omega t} + f^{(-\delta)}e^{i\delta\omega t}, \\
	\eta_{eg} &= \eta_{eg}^{(\delta)}e^{-i\delta\omega t} + \eta_{eg}^{(-\delta)}e^{i\delta\omega t}, \\
	\eta_{sg} &= \eta_{sg}^{(\delta)}e^{-i \delta\omega t } + \eta_{sg}^{(-\delta)}e^{i\delta\omega t},
	\end{align}
\end{subequations}
where $\delta\omega$ equals approximately \emph{half} the Rabi frequency ($\hbar\Omega_{se}$) of the tunneling transition. Next we use the above ansatz to evaluate the products $f^*\eta_{sg}$ and $f^*\eta_{eg}$ in Eq. (\ref{eq:threelevelmodel}). Using the index $j$ as a place holder for $eg$ and  $sg$ we obtain
\begin{widetext}
	\begin{align}
	\label{eq:feta-product}
	f^{*}\eta_{j} &= \left [ (f^{(\delta)})^*e^{  i\delta\omega t} + (f^{(-\delta)})^*e^{-i\delta\omega t}\right ] \left [ \eta_{j}^{(\delta)}e^{ - i\delta\omega t} + \eta_{j}^{(-\delta)}e^{+i\delta\omega t} \right ] \nonumber \\
	&= \big[ (f^{(\delta)})^*\eta_{j}^{(\delta)} +(f^{(-\delta)})^*\eta_{j}^{(-\delta)} + (f^{(\delta)})^*\eta_{j}^{(-\delta)}e^{2i\delta\omega t} + (f^{(-\delta)})^*\eta_{j}^{(\delta)}e^{-2i\delta\omega t}\big ].
	\end{align} 
\end{widetext}
We can reconciliate this result with Eq. (\ref{eq:threelevelmodel}) if we extend it with the following ansatz for the  population densities and the $\rho_{se}$ coherence term 
...
\rho_{se} &= \rho_{se}^{DC}+\rho_{se}^{+}e^{-2i\delta \omega t} + \rho_{se}^{-}e^{-2i\delta \omega t},
\end{align}
where $j \in \{s,e,g\}$, $\rho_{jj}^{-} = (\rho_{jj}^{(+)})^*$ and $\rho_{se}^{-} = (\rho_{es}^{+})^*$ due to the Hermitian property of the density matrix. Then, we can derive the evolution equations for $\rho_{jj}^{DC},\rho_{jj}^{\pm}$ and $\rho_{se}$ via direct substitution in  Eq. \ref{eq:threelevelmodel}
...
\begin{align}
\frac{n}{c}\partial_t f^{(\pm \delta)} + \partial_{x}f^{(\pm \delta)}&= -i\frac{N \Gamma q_0d_{eg} k_c}{\epsilon_0 n^2} \eta_{eg}^{(\pm \delta)} \nonumber \\ 
&- \left[\frac{l_0}{2}  \mp i \frac{n\delta\omega}{c}\right] f^{(\pm \delta)}\label{eq:rtwave-temphole}.
\end{align}


\section{Temporal hole burning due to strong injector anticrossing - 03 - series expansion}

As it can be easily seen, the above derivation is based on a Fourier expansion of the electric field accompanied by the same for the density matrix elements. We will generalize this approach in the following. 

First of all, let us write the von Neumann equation in the rotating wave approximation in matrix form:

\begin{align}
\label{eq:vonNeumannMatrix}
\frac{d}{dt} 
& \begin{pmatrix}
\rho_{ss} &	 \rho_{se} & \eta_{sg} \\
\rho_{es} &	 \rho_{ee} & \eta_{eg} \\
\eta_{gs} &	 \eta_{ge} & \rho_{gg} \\
\end{pmatrix} = coll \nonumber \\
& -\frac{i}{\hbar}\left [ 
\begin{pmatrix}
\hbar(\epsilon + \Delta ) & \hbar \Omega_{se} & 0 \\ 
\hbar \Omega_{se} & \hbar\Delta & \frac{\hbar}{2}\Omega_p \\
0 & \frac{\hbar}{2}\Omega_p^* & 0
\end{pmatrix}; 
\begin{pmatrix}
\rho_{ss} &	 \rho_{se} & \eta_{sg} \\
\rho_{es} &	 \rho_{ee} & \eta_{eg} \\
...
\end{pmatrix} \right ],
\end{align}
where the Rabi frequency is defined as $\Omega_p = \frac{q_0d_{eg}}{\hbar}f$.

We expand the electric field envelope as follows
\begin{equation}
\Omega_p = \sum_{m=-p}^{p} f^{(p)}e^{-ip\delta\omega t}, 
\end{equation}
from where we conclude that also the density matrix ought to contain proportional terms and also terms oscillating $\propto \exp(\pm2i\delta\omega t)$, because the population inversion follows the intensity of the electric field rather than its envelope.  This justifies the following ansatz for the density matrix elements 

\begin{equation}
\rho = \sum_{m = -p}^{p} c_m \rho^{m}e^{-im\delta\omega t} + c_{2m}\rho^{2m}e^{-i2m\delta\omega t} , 
\end{equation}
where $c_m$ are suitably chosen constants intended to prevent double counting. Combining all of the above, we obtain the following equations of motion for the  density matrix elements
